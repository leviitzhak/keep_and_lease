...
self.onmessage=async({data})=>{if(!data||data.type!=="run")return;try{stage="running the requested backtest";pyodide.globals.set("request_json",JSON.stringify(data.payload||{}));const responseJson=await pyodide.runPythonAsync(`
import json
