  ...
  template {
    ...
    containers {
      ...
      startup_probe {
        timeout_seconds = 2
        period_seconds  = 2
        # Keep each TCP check strict while allowing a cold instance up to two
        # minutes to bind. The deployment workflow validates the authenticated
        # HTTP health payload separately before running GUI smoke tests.
