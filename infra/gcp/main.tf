...
resource "google_storage_bucket" "market_data" {
  name                        = "${var.project_id}-market-data"
  location                    = var.region
...
resource "google_service_account" "worker" {
  account_id   = "keep-lease-worker"
  display_name = "Keep & Lease calculation worker"
}

resource "google_service_account" "deploy" {
  account_id   = "keep-lease-github"
  display_name = "Keep & Lease GitHub deployment"
}

resource "google_storage_bucket_iam_member" "worker_market_reader" {
  bucket = google_storage_bucket.market_data.name
  role   = "roles/storage.objectViewer"
  member = "serviceAccount:${google_service_account.worker.email}"
}

resource "google_storage_bucket_iam_member" "worker_results_writer" {
  bucket = google_storage_bucket.results.name
  role   = "roles/storage.objectUser"
  member = "serviceAccount:${google_service_account.worker.email}"
}

resource "google_storage_bucket_iam_member" "web_results_reader" {
  bucket = google_storage_bucket.results.name
  role   = "roles/storage.objectViewer"
...
resource "google_service_account_iam_member" "deploy_use_web" {
  service_account_id = google_service_account.web.name
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${google_service_account.deploy.email}"
}

resource "google_service_account_iam_member" "deploy_use_worker" {
