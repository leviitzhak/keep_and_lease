...
The 90-day BTC subsecond implementation and operational workflow history are
merged into GitHub master. The verified June 6–September 4 range has completed
both ordering-policy benchmarks. Saved results and selected-period XLSX exports
are available in the GUI; full-period execution acceptance on Cloud Run remains
separate from the completed GitHub Actions benchmarks.

The permanent `agent/cloud-autonomous-access` branch remains the authenticated
request entry point. Its request files are retained as operational history;
merging them into master does not trigger the branch-restricted BTC workflows.
...
peak process RSS: 653.836 MiB; compressed audit: 7,445,502,103 bytes. The timestamp policy and paired comparison subsequently completed at 18:41 UTC
on September 9. Both completed benchmarks can be opened in the GUI, with
full-resolution selected-period XLSX exports (see `BTC_SUBSECOND_GUI.md`).
